...
for set_fuzzy in sets_fuzzy:
    ...
    ax1.set_ylabel('RMSE (cycles)', color=color)
    bars = ax1.bar(np.array(features) - 0.2, rmse, color=color, alpha=0.7, width=0.4, label='RMSE')
    ax1.tick_params(axis='y', labelcolor=color)
    ax1.set_ylim((17, 37))
    # Adicionar números sobre as barras de RMSE
    for bar, value in zip(bars, rmse):
        yval = bar.get_height()
        ax1.text(bar.get_x() + bar.get_width() / 2, yval, round(value, 2), ha='center', va='bottom', color='black')

    # Criar eixo y da direita (Quantidade de Regras)
    ax2 = ax1.twinx()
    color = 'tab:blue'
    ax2.set_ylabel('Number of Rules', color=color)
    ax2.bar(np.array(features) + 0.2, num_rules, width=0.4, color=color, alpha=0.7, label='Number of Rules')
